...
class Snake:
    ...
    def move(self, key):
        if abs(key) != self.last_key:
            if key == 1: self.step = [0,-1]
            elif key == -1: self.step = [0,1]
            elif key == 2: self.step = [-1,0]
            elif key == -2: self.step = [1,0]
            self.last_key = abs(key) if key != 0 else self.last_key

        self.head = [self.head[0]+self.step[0], self.head[1]+self.step[1]]
        if self.good_move(self.head):
    ...
    def gen_apple(self):
        while True:
            self.apple = [random.randint(1,self.board_size[0]), random.randint(1,self.board_size[1])]
            if self.apple not in self.body: break

    def good_move(self, head):
